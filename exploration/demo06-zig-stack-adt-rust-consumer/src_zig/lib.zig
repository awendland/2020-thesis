...
export fn entry_point() i32 {
  ...
  // NOTE: originally suffixed with `catch unreachable` when Stack.push propagated its error values
  defer stack1.deinit();
  stack1.push(-3);
...
pub const Stack = struct {
  ...
  pub fn push(self: *Self, item: i32) void {
    ...
    // Originally followed Zig convention of `try` macro. However, that returns an ErrorSet
    // which causes the return type to no longer simply be `void`, it must be `!void` (which is
    // sugared). This caused the Wasm type to be `(i32, i32) -> i32`, with the returned i32 likely
    // being used as a pointer to error data on the heap.
    //
    // TODO: need to determine how to propagate errors between languages.
    //
    // `try self.ensureCapacity(new_length);`
    self.ensureCapacity(new_length) catch unreachable;
